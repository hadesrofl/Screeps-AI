...
var roleDefender = {
  parts: [ATTACK, TOUGH, TOUGH, MOVE, MOVE, MOVE,
    MOVE],
  bigParts: [RANGED_ATTACK, ATTACK, TOUGH, TOUGH, TOUGH, TOUGH, TOUGH,
    ...
    // not in sight
      ...
      if (enemy) {
        ...
        if (creep.memory.defendExit != 0) {
          ...
          var exitsInRange = creep.pos.findInRange(dir, 5);
          if (exitsInRange.length) {
            creep.memory.arrived = true;
            return;
          } else {
            creep.memory.arrived = false;
            creep.moveTo(creep.pos.findClosestByRange(dir));
          }
        } else {
        ...
        /**if (creep.room.name != "W84S8") {
          var route = Game.map.findRoute(creep.room, "W84S8");
          if (route.length > 0) {
